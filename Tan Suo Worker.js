<script type="text/javascript" type="text/worker">
    ...
    on("change:acrobat_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["acrobat"]);
    });

    on("change:acting_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["acting"]);
    });

    on("change:archivist_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["archivist"]);
    });

    on("change:athlete_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["athlete"]);
    });

    on("change:bouncer_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["bouncer"]);
    });

    on("change:counsel_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["counsel"]);
    });

    on("change:detective_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["detective"]);
    });

    on("change:hide_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["hide"]);
    });

    on("change:magic_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["magic"]);
    });

    on("change:scout_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["scout"]);
    });

    on("change:speaking_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["speaking"]);
    });

    on("change:terrain_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["terrain"]);
    });

    on("change:theft_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["theft"]);
    });

    on("change:trailblazer_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["trailblazer"]);
    });

    on("change:wellness_training_days", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_training(["wellness"]);
    });

    on("change:lifestyle_location change:lifestyle_residence change:lifestyle_residence_purchased change:repeating_expansions:expansion_name change:repeating_expansions:expansion_upkeep", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_lifestyle();
    });

    on("change:repeating_levelUpdates:levelClassName change:repeating_levelUpdates:HpRoll change:repeating_levelUpdates:BarrierRoll", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
    ...
    on("change:emote_name", function(eventinfo) {
        if(eventinfo.sourceType === "sheetworker") {return;};
        update_emotes();
    });
    ...
    on("change:repeating_inventory:itemweight change:repeating_inventory:itemcount change:jin change:frt change:syr change:gp change:cp change:encumberance_setting change:size change:carrying_capacity_mod", function() {
        update_weight();
    });
    ...
    on("change:repeating_spell-cantrip:spelloutput change:repeating_spell-1:spelloutput change:repeating_spell-2:spelloutput change:repeating_spell-3:spelloutput change:repeating_spell-4:spelloutput change:repeating_spell-5:spelloutput change:repeating_spell-6:spelloutput change:repeating_spell-7:spelloutput change:repeating_spell-8:spelloutput change:repeating_spell-9:spelloutput", function(eventinfo) {
        ...
        getAttrs([repeating_source + "_spellattackid", repeating_source + "_spelllevel", repeating_source + "_spelloutput", repeating_source + "_spellattackid", repeating_source + "_spellathigherlevels","character_id"], function(v) { 
            ...
            if(spelloutput && spelloutput === "ATTACK") {
                ...
                update[repeating_source + "_rollcontent"] = "@{wtype}&{template:spell} {{level=@{spellschool} " + spelllevel + "}} {{name=@{spellname}}} {{castingtime=@{spellcastingtime}}} {{range=@{spellrange}}} {{duration=@{spellduration}}} {{description=@{spelldescription}}} {{athigherlevels=@{spellathigherlevels}}} @{spellritual} {{innate=@{innate}}} @{spellconcentration} @{charname_output}";
            }
    ...
    on("change:repeating_npcaction:attack_flag change:repeating_npcaction:attack_type change:repeating_npcaction:attack_range change:repeating_npcaction:attack_target change:repeating_npcaction:attack_tohit change:repeating_npcaction:attack_damage change:repeating_npcaction:attack_damagetype change:repeating_npcaction:attack_element change:repeating_npcaction:attack_damage2 change:repeating_npcaction:attack_damagetype2 change:repeating_npcaction:attack_element2 change:repeating_npcaction-l:attack_flag change:repeating_npcaction-l:attack_type change:repeating_npcaction-l:attack_range change:repeating_npcaction-l:attack_target change:repeating_npcaction-l:attack_tohit change:repeating_npcaction-l:attack_damage change:repeating_npcaction-l:attack_damagetype change:repeating_npcaction-l:attack_element change:repeating_npcaction-l:attack_damage2 change:repeating_npcaction-l:attack_damagetype2 change:repeating_npcaction-l:attack_element2 change:repeating_npcaction:show_desc change:repeating_npcaction-l:show_desc change:repeating_npcaction:description change:repeating_npcaction-l:description", function(eventinfo) {
    ...
    var update_training = function (attr) {
        console.log("UPDATING TRAINING: " + attr);
        var training_attrs = [attr + "_training_days", "attr_base_level", "attr_multiclass1_lvl", "attr_multiclass2_lvl", "attr_multiclass3_lvl"];
    
        getAttrs(training_attrs, function(v) {
            var training_days = v[attr + "_training_days"] ? parseInt(v[attr + "_training_days"], 10) : 0;
            
            var level = 0;
            level += v["attr_base_level"] ? parseInt(v["attr_base_level"], 10) : 0;
            level += v["attr_multiclass1_lvl"] ? parseInt(v["attr_multiclass1_lvl"], 10) : 0;
            level += v["attr_multiclass2_lvl"] ? parseInt(v["attr_multiclass2_lvl"], 10) : 0;
            level += v["attr_multiclass3_lvl"] ? parseInt(v["attr_multiclass3_lvl"], 10) : 0;

            var tier = 0;
            if (training_days >= 1080 && level >= 12) {
                tier = 4;
            }
            if (training_days >= 480 && level >= 8) {
                tier = 3;
            }
            else if (training_days >= 200 && level >= 4) {
                tier = 2;
            }
            else if (training_days >= 60) {
                tier = 1;
            }
            var update = {};
            update[attr + "_tier"] = tier;
            setAttrs(update, {silent: true});
        });
    };
    ...
    var update_health_barrier = function ()
    {
        ...
        var health_attrs = ["hp_max", "barrier_max", "constitution_mod"];
        
        var totalHP = 0;
        var totalBarrier = 0;
        
        getSectionIDs("repeating_levelUpdates", function(idarray) {
            _.each(idarray, function(currentID, i) {
                health_attrs.push("repeating_levelUpdates_" + currentID + "_levelClassName");
                health_attrs.push("repeating_levelUpdates_" + currentID + "_HpRoll");
                health_attrs.push("repeating_levelUpdates_" + currentID + "_HpMod");
                health_attrs.push("repeating_levelUpdates_" + currentID + "_HpRollTotal");
                health_attrs.push("repeating_levelUpdates_" + currentID + "_BarrierRoll");
                health_attrs.push("repeating_levelUpdates_" + currentID + "_BarrierMod");
                health_attrs.push("repeating_levelUpdates_" + currentID + "_BarrierRollTotal");
            });
            ...
            getAttrs(health_attrs, function(v) {
                ...
                _.each(idarray, function(currentID) {
                    var hpMod = 0;
                    switch (v["repeating_levelUpdates_" + currentID + "_levelClassName"])
                    {
                    case "Arcanist":
                    case "Spiritualist":
                    case "Wizard":
                    ...
                    var listHP = v["repeating_levelUpdates_" + currentID + "_HpRoll"] ? parseInt(v["repeating_levelUpdates_" + currentID + "_HpRoll"], 10) : 0;
                    listHP += hpMod;
                    totalHP += listHP;
                    
                    var listBarrier = v["repeating_levelUpdates_" + currentID + "_BarrierRoll"] ? parseInt(v["repeating_levelUpdates_" + currentID + "_BarrierRoll"], 10) : 0;
                    listBarrier += v["constitution_mod"];
                    totalBarrier += listBarrier;
                    
                    
                    update = {};
                    update["repeating_levelUpdates_" + currentID + "_HpMod"] = hpMod;
                    update["repeating_levelUpdates_" + currentID + "_HpRollTotal"] = listHP;
                    update["repeating_levelUpdates_" + currentID + "_BarrierMod"] = v["constitution_mod"] + " (CON)";
                    update["repeating_levelUpdates_" + currentID + "_BarrierRollTotal"] = listBarrier;
                    setAttrs(update, {silent: true});
                });
            
                update = {};
                update["hp_max"] = totalHP;
                update["barrier_max"] = totalBarrier;
                setAttrs(update, {silent: true});
            });
        });
    };
    ...
    var update_emotes = function ()
    {
        ...
        var update = {};
        var attr_fields = ["attr_emote_count"];
        getSectionIDs("repeating_emotes", function(idarray) {
            _.each(idarray, function(currentID, i) {
                attr_fields.push("repeating_emotes_" + currentID + "_url");
            });
    ...
    var update_lifestyle = function ()
    {
        ...
        var lifestyle_attrs = ["lifestyle_location", "lifestyle_location_price", "lifestyle_location_quality", "lifestyle_residence", "lifestyle_residence_expansions", "lifestyle_residence_price", "lifestyle_residence_purchased"];
    
        var totalCost = 0;
        
        getSectionIDs("repeating_expansions", function(idarray) {
            _.each(idarray, function(currentID, i) {
                lifestyle_attrs.push("repeating_expansions_" + currentID + "_expansion_upkeep");
            });
            
            getAttrs(lifestyle_attrs, function(v) {

                var resSelection = v["lifestyle_residence"];
                var resBasePrice = 0;
                var resPrice = 0;
                var resExpansions = "0 Expansions";

                switch(resSelection)
                {
                case "Street":
                    resBasePrice = 0;
                    resExpansions = "0 Expansions";
                    break;
                case "Poor":
                    resBasePrice = 600;
                    resExpansions = "1 Expansion";
                    break;
                case "Low":
                    resBasePrice = 1200;
                    resExpansions = "2 Expansions";
                    break;
                case "Middle":
                    resBasePrice = 2400;
                    resExpansions = "3 Expansions";
                    break;
                case "High":
                    resBasePrice = 4200;
                    resExpansions = "4 Expansions";
                    break;
                case "Aristocratic":
                    resBasePrice = 8800;
                    resExpansions = "6 Expansions";
                    break;
                case "Luxury":
                    resBasePrice = 20000;
                    resExpansions = "8 Expansions";
                    break;
                }
                
                if (v["lifestyle_residence_purchased"] != 0)
                {
                    resPrice = 0;
                }
                else
                {
                    resPrice = resBasePrice;
                }

                var locationSelection = v["lifestyle_location"];
                var locPrice = 0;
                var locQuality = "None";
                ...
                case "Calisto":
                    locPrice = 1800;
                    locQuality = "Aristocratic – Luxury";
                    break;
                case "Ganymede":
                    locPrice = 0;
                    locQuality = "Street";
                ...
                case "Etrusca":
                    locPrice = 1500;
                    locQuality = "High – Luxury";
                    break;
                case "Hephaes":
                    locPrice = 400;
                    locQuality = "Low – Middle";
                    break;
                case "Martius":
                    locPrice = 400;
                    locQuality = "Low – Middle";
                ...
                case "Geminus":
                    locPrice = 1500;
                    locQuality = "Middle – Aristocratic";
                    break;
                case "Iunon":
                    locPrice = 1800;
                    locQuality = "High – Luxury";
                ...
                if (resSelection == "Street")
                {
                    locPrice = 0;
                }
                
                var update = {};
                update["lifestyle_location_price"] = "" + locPrice + "";
                update["lifestyle_location_quality"] = locQuality;
                if (v["lifestyle_residence_purchased"] != 0)
                {
                    update["lifestyle_residence_price"] = "" + resPrice + " (" + resBasePrice + ")";
                }
                else
                {
                    update["lifestyle_residence_price"] = "" + resPrice + "";
                }
                update["lifestyle_residence_expansions"] = resExpansions;

                totalCost = locPrice + resPrice;
                _.each(idarray, function(currentID) {
                    totalCost += v["repeating_expansions_" + currentID + "_expansion_upkeep"] ? parseInt(v["repeating_expansions_" + currentID + "_expansion_upkeep"], 10) : 0;
                });
    ...
    var handle_drop = function(category, eventinfo) {
        ...
        getAttrs(["drop_name","drop_weight","drop_properties","drop_modifiers","drop_content","drop_itemtype","drop_damage","drop_damagetype","drop_damage2","drop_damagetype2","drop_range","drop_ac","drop_spellritualflag", "drop_spellschool", "drop_spellcastingtime", "drop_spellconcentrationflag", "drop_spellduration", "drop_spellhealing", "drop_spelldmgmod", "drop_spellsave", "drop_spellsavesuccess", "drop_spellhldie", "drop_spellhldietype", "drop_spellhlbonus", "drop_spelllevel", "drop_spell_damage_progression", "drop_attack_type", "drop_speed", "drop_str", "drop_dex", "drop_con", "drop_int", "drop_wis", "drop_cha", "drop_vulnerabilities", "drop_resistances", "drop_immunities", "drop_condition_immunities", "drop_languages", "drop_challenge_rating", "drop_size", "drop_type", "drop_alignment", "drop_hp", "drop_saving_throws", "drop_senses", "drop_passive_perception", "drop_skills", "drop_token_size", "character_id"], function(v) {
            if(category === "Items") {
                update["tab"] = "core";
            ...
            if(category === "Spells") {
                ...
                if(v.drop_damage || v.drop_spellhealing) {
                    ...
                    spelllevel = spelllevel + "}";
                    update["repeating_spell-" + lvl + "_" + id + "_rollcontent"] = "@{wtype}&{template:spell} {{level=@{spellschool} " + spelllevel + "}} {{name=@{spellname}}} {{castingtime=@{spellcastingtime}}} {{range=@{spellrange}}} {{duration=@{spellduration}}} {{description=@{spelldescription}}} {{athigherlevels=@{spellathigherlevels}}} @{spellritual} {{innate=@{innate}}} @{spellconcentration} @{charname_output}";
                };
    ...
    var update_attack_from_spell = function(lvl, spellid, attackid, newattack) {
        getAttrs(["repeating_spell-" + lvl + "_" + spellid + "_spellname", "repeating_spell-" + lvl + "_" + spellid + "_spellrange", "repeating_spell-" + lvl + "_" + spellid + "_spellattack", "repeating_spell-" + lvl + "_" + spellid + "_spelldamage", "repeating_spell-" + lvl + "_" + spellid + "_spelldamage2", "repeating_spell-" + lvl + "_" + spellid + "_spelldamagetype", "repeating_spell-" + lvl + "_" + spellid + "_spelldamagetype2", "repeating_spell-" + lvl + "_" + spellid + "_spellhealing", "repeating_spell-" + lvl + "_" + spellid + "_spelldmgmod", "repeating_spell-" + lvl + "_" + spellid + "_spellsave", "repeating_spell-" + lvl + "_" + spellid + "_spellsavesuccess", "repeating_spell-" + lvl + "_" + spellid + "_spellhldie", "repeating_spell-" + lvl + "_" + spellid + "_spellhldietype", "repeating_spell-" + lvl + "_" + spellid + "_spellhlbonus", "repeating_spell-" + lvl + "_" + spellid + "_spelllevel", "repeating_spell-" + lvl + "_" + spellid + "_includedesc", "repeating_spell-" + lvl + "_" + spellid + "_spelldescription", "repeating_spell-" + lvl + "_" + spellid + "_spellathigherlevels", "repeating_spell-" + lvl + "_" + spellid + "_spell_damage_progression", "repeating_spell-" + lvl + "_" + spellid + "_innate", "spellcasting_ability"], function(v) {
            var update = {};
            var description = "";
    ...
    var do_update_attack = function(attack_array, source) {
        console.log("UPDATING ATTACK with do_update_attack");
        ...
        _.each(attack_array, function(attackid) {
            ...
            attack_attribs.push("repeating_attack_" + attackid + "_dmgelement");
            ...
            attack_attribs.push("repeating_attack_" + attackid + "_dmg2element");
        ...
        getAttrs(attack_attribs, function(v) {
            _.each(attack_array, function(attackid) {
                ...
                var dmgelement = v["repeating_attack_" + attackid + "_dmgelement"] ? v["repeating_attack_" + attackid + "_dmgelement"] + " " : "";
                var dmg2element = v["repeating_attack_" + attackid + "_dmg2element"] ? v["repeating_attack_" + attackid + "_dmg2element"] + " " : "";
                ...
                if(v["repeating_attack_" + attackid + "_dmg2flag"] && v["repeating_attack_" + attackid + "_dmg2flag"] != 0) {
                    ...
                    dmg2 = dmg2 + " " + dmg2type + dmg2element;
                }
                else {
                    dmg2 = "";
                };
                ...
                if(v.dtype === "full") {
                    pickbase = "full";
                    rollbase = "@{wtype}&{template:atkdmg} {{mod=@{atkbonus}}} {{rname=@{atkname}}} {{r1=[[" + r1 + "cs>@{atkcritrange}" + hbonus + "]]}} " + r2 + "cs>@{atkcritrange}" + hbonus + "]]}} @{atkflag} {{range=@{atkrange}}} @{dmgflag} {{dmg1=[[" + hdmg1 + "]]}} {{dmg1type=" + dmgtype + "}} {{dmg1element=" + dmgelement + "}} @{dmg2flag} {{dmg2=[[" + hdmg2 + "]]}} {{dmg2type=" + dmg2type + "}} {{dmg2element=" + dmg2element + "}} {{crit1=[[" + crit1 + "[CRIT]]]}} {{crit2=[[" + crit2 + "[CRIT]]]}} @{saveflag} {{desc=@{atk_desc}}} @{hldmg} " + hldmgcrit + " {{spelllevel=@{spelllevel}}} {{innate=@{spell_innate}}} @{global_attack_mod} @{global_damage_mod} @{global_damage_mod_crit} {{globaldamagetype=@{global_damage_type}}} ammo=@{ammo} @{charname_output}";
                }
                    ...
                    pickbase = "dmg";
                    rollbase = "@{wtype}&{template:dmg} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{dmgflag} {{dmg1=[[" + hdmg1 + "]]}} {{dmg1type=" + dmgtype + "}} {{dmg1element=" + dmgelement + "}} @{dmg2flag} {{dmg2=[[" + hdmg2 + "]]}} {{dmg2type=" + dmg2type + "}} {{dmg2element=" + dmg2element + "}} @{saveflag} {{desc=@{atk_desc}}} @{hldmg} {{spelllevel=@{spelllevel}}} {{innate=@{spell_innate}}} @{global_damage_mod} {{globaldamagetype=@{global_damage_type}}} ammo=@{ammo} @{charname_output}"
                }
                else {
                    pickbase = "empty";
                ...
                update["repeating_attack_" + attackid + "_rollbase_dmg"] = "@{wtype}&{template:dmg} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{dmgflag} {{dmg1=[[" + hdmg1 + "]]}} {{dmg1type=" + dmgtype + "}} {{dmg1element=" + dmgelement + "}} @{dmg2flag} {{dmg2=[[" + hdmg2 + "]]}} {{dmg2type=" + dmg2type + "}} {{dmg2element=" + dmg2element + "}} @{saveflag} {{desc=@{atk_desc}}} @{hldmg} {{spelllevel=@{spelllevel}}} {{innate=@{spell_innate}}} @{global_damage_mod} {{globaldamagetype=@{global_damage_type}}} @{charname_output}";
                update["repeating_attack_" + attackid + "_rollbase_crit"] = "@{wtype}&{template:dmg} {{crit=1}} {{rname=@{atkname}}} @{atkflag} {{range=@{atkrange}}} @{dmgflag} {{dmg1=[[" + hdmg1 + "]]}} {{dmg1type=" + dmgtype + "}} {{dmg1element=" + dmgelement + "}} @{dmg2flag} {{dmg2=[[" + hdmg2 + "]]}} {{dmg2type=" + dmg2type + "}} {{dmg2element=" + dmg2element + "}} {{crit1=[[" + crit1 + "]]}} {{crit2=[[" + crit2 + "]]}} @{saveflag} {{desc=@{atk_desc}}} @{hldmg} " + hldmgcrit + " {{spelllevel=@{spelllevel}}} {{innate=@{spell_innate}}} @{global_damage_mod} @{global_damage_mod_crit} {{globaldamagetype=@{global_damage_type}}} @{charname_output}"
    ...
    var update_spell_from_attack = function(lvl, spellid, attackid) {
        var update = {};
        getAttrs(["repeating_attack_" + attackid + "_atkname", "repeating_attack_" + attackid + "_atkrange", "repeating_attack_" + attackid + "_dmgbase", "repeating_attack_" + attackid + "_dmgtype", "repeating_attack_" + attackid + "_dmgelement", "repeating_attack_" + attackid + "_dmg2base", "repeating_attack_" + attackid + "_dmg2type", "repeating_attack_" + attackid + "_dmg2element", "repeating_attack_" + attackid + "_saveflag", "repeating_attack_" + attackid + "_saveattr", "repeating_attack_" + attackid + "_saveeffect"], function(v) {
    ...
    var update_weight = function() {
        var update = {};
        var wtotal = 0;
        var weight_attrs = ["jin","frt","syr","gp","cp","encumberance_setting","strength","size","carrying_capacity_mod"];
        getSectionIDs("repeating_inventory", function(idarray) {
            _.each(idarray, function(currentID, i) {
            ...
            getAttrs(weight_attrs, function(v) {
                ...
                wtotal = wtotal + ((gp + cp) / 50) + (syr / 100) + ((jin + frt) / 1000);
                ...
                switch(v.class) {
                    ...
                    case "Arcanist":
                        ...
                        if(!v.dexterity_save_prof || v.dexterity_save_prof != "(@{pb})" || !v.charisma_save_prof || v.charisma_save_prof != "(@{pb})") {
                            ...
                            update["class_resource_name"] = "Arcane Points";
                        }
                        break;
                    case "Fighter":
                        update["hitdietype"] = 6;
                    ...
                    case "Martial-Artist":
                        ...
                        if(!v.strength_save_prof || v.strength_save_prof != "(@{pb})" || !v.dexterity_save_prof || v.dexterity_save_prof != "(@{pb})") {
                            ...
                            update["wisdom_save_prof"] = 0;
                            update["charisma_save_prof"] = 0;
                            update["class_resource_name"] = "Maneuvers";
                        }
                        break;
                    case "Ranger":
                        update["hitdietype"] = 4;
                    ...
                    case "Spellblade":
                        ...
                        if(!v.wisdom_save_prof || v.wisdom_save_prof != "(@{pb})" || !v.charisma_save_prof || v.charisma_save_prof != "(@{pb})") {
                            ...
                            update["charisma_save_prof"] = "(@{pb})";
                            update["class_resource_name"] = "Cleanse";
                        }
                        break;
                    case "Spiritualist":
                        update["hitdietype"] = 4;
    ...
    var checkCasterLevel = function(class_string, levels, arcane_fighter, arcane_rogue) {
        var full = ["arcanist","spiritualist","wizard","full"];
        var half = ["spellblade","ranger","half"];
    ...
    var checkHitDie = function(class_string) {
        var d8class = ["warrior"];
        var d6class = ["fighter","spellblade"];
        var d4class = ["arcanist","martial-artist","ranger","rogue","spiritualist","wizard"];
        if (d8class.indexOf(class_string) != -1) {return "8"}
        else if (d6class.indexOf(class_string) != -1) {return "6"}
        else if (d4class.indexOf(class_string) != -1) {return "4"}
        else {return "0"};
    };
    ...
    var do_update_npc_action = function(action_array, legendary) {
        ...
        getAttrs(action_attribs, function(v) {
            _.each(action_array, function(actionid) {
                ...
                var dmg1element = v[repvar + actionid + "_attack_element"] && v[repvar + actionid + "_attack_element"] != "" ? " " + v[repvar + actionid + "_attack_element"] : "";
                ...
                var dmg2element = v[repvar + actionid + "_attack_element2"] && v[repvar + actionid + "_attack_element2"] != "" ? " " + v[repvar + actionid + "_attack_element2"] : "";
                var dmgspacer = dmg1 != "" && dmg2 != "" ? " plus " : "";
                if(dmg1 != "") {
                    dmg1t = dmg1.replace(/\s/g, '').split(/d|(?=\+|\-)/g);
                    dmg1t2 = isNaN(eval(dmg1t[1])) === false ? eval(dmg1t[1]) : 0;
                    if(dmg1t.length < 2) {
                        onhit = onhit + dmg1t[0] + " (" + dmg1 + ")" + dmg1type + dmg1element + " damage";
                    }
                    else if(dmg1t.length < 3) {
                        onhit = onhit + Math.floor(dmg1t[0]*((dmg1t2/2)+0.5)) + " (" + dmg1 + ")" + dmg1type + dmg1element + " damage";
                    }
                    else {
                        onhit = onhit + (Math.floor(dmg1t[0]*((dmg1t2/2)+0.5))+parseInt(dmg1t[2],10)) + " (" + dmg1 + ")" + dmg1type + dmg1element + " damage";
                    };
                };
                ...
                if(dmg2 != "") {
                    ...
                    if(dmg2t.length < 2) {
                        onhit = onhit + dmg2t[0] + " (" + dmg2 + ")" + dmg2type + dmg2element + " damage";
                    }
                    else if(dmg2t.length < 3) {
                        onhit = onhit + Math.floor(dmg2t[0]*((dmg2t2/2)+0.5)) + " (" + dmg2 + ")" + dmg2type + dmg2element + " damage";
                    }
                    else {
                        onhit = onhit + (Math.floor(dmg2t[0]*((dmg2t2/2)+0.5))+parseInt(dmg2t[2],10)) + " (" + dmg2 + ")" + dmg2typ + dmg2element + " damage";
                    };
                };
                ...
                var rollbase = "";
                if(v.dtype === "full") {
                    rollbase = "@{wtype}&{template:npcaction} " + attack_flag + " @{damage_flag} @{npc_name_flag} {{rname=@{name}}} {{r1=[[@{d20}+(@{attack_tohit}+0)]]}} @{rtype}+(@{attack_tohit}+0)]]}} {{dmg1=[[@{attack_damage}+0]]}} {{dmg1type=@{attack_damagetype}}} {{dmg1element=@{attack_element}}} {{dmg2=[[@{attack_damage2}+0]]}} {{dmg2type=@{attack_damagetype2}}} {{dmg2element=@{attack_element2}}} {{crit1=[[@{attack_crit}+0]]}} {{crit2=[[@{attack_crit2}+0]]}} {{description=@{show_desc}}} @{charname_output}";
                }
                    ...
                    rollbase = "@{wtype}&{template:npcdmg} @{damage_flag} {{dmg1=[[@{attack_damage}+0]]}} {{dmg1type=@{attack_damagetype}}} {{dmg1element=@{attack_element}}} {{dmg2=[[@{attack_damage2}+0]]}} {{dmg2type=@{attack_damagetype2}}} {{dmg2element=@{attack_element2}}} {{dmgelement=@{attack_element}}} {{crit1=[[@{attack_crit}+0]]}} {{crit2=[[@{attack_crit2}+0]]}} @{charname_output}"
                }
                else {
