...
var CombatMaster = CombatMaster || (function() {
    ...
    inputHandler = function(msg_orig) {
        ...
        if (msg.content.indexOf('import') >= 0) {
            ...
            msg.content = msg.content.replace('!cm ','')
...
//*************************************************************************************************************
    sendMainMenu = function(who) {
        ...
        let prevButton          = makeImageButton('!cm --turn,previous',prevImage,'Previous Turn','transparent',18)
        let stopButton          = makeImageButton('!cm --turn,stop --main',stopImage,'Stop Combat','transparent',18)
        let holdButton          = makeImageButton('!cm --turn,hold',holdImage,'Hold Combat','transparent',18)
        let startButton         = makeImageButton('!cm --turn,start --main',startImage,'Start Combat','transparent',18)
        let pauseTimerButton    = makeImageButton('!cm --turn,timer=pause',pauseImage,'Pause Timer','transparent',18)
        let stopTimerButton     = makeImageButton('!cm --turn,timer=stop',timerImage,'Stop Timer','transparent',18)
        let configButton        = makeImageButton('!cm --show,setup',backImage,'Show Setup','transparent',18)
        let showButton          = makeImageButton('!cm --show,assigned',showImage,'Show Conditions','transparent',18)
        let sortButton          = makeImageButton('!cm --turn,sort',sortImage,'Sort Turnorder','transparent',18)
        let helpButton
        
        if (state[combatState].config.hold.held) {
            helpButton          = makeImageButton('!cm --help,held',helpImage,'Help','transparent',18,'white')
        } else if (inFight() ) { 
            helpButton          = makeImageButton('!cm --help,started',helpImage,'Help','transparent',18,'white')
        } else {
            helpButton          = makeImageButton('!cm --help,stopped',helpImage,'Help','transparent',18,'white')
        }    
        ...
        let viewButton = makeBigButton('Change View', '!cm --show,view,value=?{View|All,all|Conditions,conditions|Spells,spells|Favorites,favorites} --main')
    ...
    sendConfigMenu = function() {
	    ...
	    let configTurnorderButton       = makeBigButton('Turnorder', '!cm --show,turnorder')
		let	configTimerButton           = makeBigButton('Timer', '!cm --show,timer')
		let	configAnnouncementsButton   = makeBigButton('Announce', '!cm --show,announce')
		let	configMacroButton           = makeBigButton('Macro & API', '!cm --show,macro')
		let	configConcentrationButton   = makeBigButton('Concentration', '!cm --show,concentration')
		let	configStatusButton          = makeBigButton('Status', '!cm --show,status')
		let	configConditionButton       = makeBigButton('Conditions', '!cm --show,conditions')
		let	exportButton                = makeBigButton('Export', '!cm --show,export')
		let	importButton                = makeBigButton('Import', '!cm --import,config=?{Config}')	
		let	resetButton                 = makeBigButton('Reset', '!cm --reset')
		let	ignoreButton                = makeBigButton('Remove Ignores', '!cm --ignore')
		let	clearButton                 = makeBigButton('Clear Token Statuses', '!cm --clear')
		let helpButton                  = makeImageButton('!cm --help,setup',helpImage,'Help','transparent',18,'white')
		let	backToTrackerButton         = makeBigButton('Back', '!cm --back,tracker')
	...
	sendTurnorderMenu = function() {
    	...
    	listItems.push(makeTextButton('Marker Size',turnorder.markerSize, '!cm --config,turnorder,key=markerSize,value=?{Marker Size (1.35 default)} --show,turnorder'))      
        listItems.push(makeTextButton('Animate Marker',turnorder.animateMarker, '!cm --config,turnorder,key=animateMarker,value='+!turnorder.animateMarker + ' --show,turnorder'))    	  
    	listItems.push(makeTextButton('Animation Angle Step',turnorder.animateMarkerDegree, '!cm --config,turnorder,key=animateMarkerDegree,value=?{Degrees to rotate every tick (15 default)} --show,turnorder'))      
    	listItems.push(makeTextButton('Animation Angle Wait',turnorder.animateMarkerWait, '!cm --config,turnorder,key=animateMarkerWait,value=?{milliseconds per tick (250 default)} --show,turnorder'))         
	...
	sendMacroMenu = function() {
        ...
        let addButton = makeBigButton('Add Substiution', '!cm --new,macro,type=?{Type|CharID,CharID|CharName,CharName|TokenID,TokenID|PlayerID,PlayerID},action=?{Action|}')
        let substitutions = state[combatState].config.macro.substitutions
        let listItems=[],contents,deleteButton,listContents
  
        substitutions.forEach((substitution) => {
            deleteButton = makeImageButton('!cm --delete,macro,action='+substitution.action,deleteImage,'Delete Substitution','transparent',12)
	...
	sendStatusMenu = function() {
        ...
        let listItems = [
				makeTextButton('Whisper GM Only', status.sendOnlyToGM, '!cm --config,status,key=sendOnlyToGM,value='+!status.sendOnlyToGM+' --show,status'),
				makeTextButton('Player Allowed Changes', status.userChanges, '!cm --config,status,key=userChanges,value='+!status.userChanges+' --show,status'),
				makeTextButton('Send Changes to Chat', status.sendConditions, '!cm --config,status,key=sendConditions,value='+!status.sendConditions+' --show,status'),	
				makeTextButton('Clear Conditions on Close', status.clearConditions, '!cm --config,status,key=clearConditions,value='+!status.clearConditions + ' --show,status'),
				makeTextButton('Use Messages', status.useMessage, '!cm --config,status,key=useMessage,value='+!status.useMessage + ' --show,status'),
				makeTextButton('Auto Add Spells', status.autoAddSpells, '!cm --config,status,key=autoAddSpells,value='+!status.autoAddSpells+' --show,status'),
		]	

        if (status.autoAddSpells) {
            listItems.push(makeTextButton('Sheet', status.sheet, '!cm --config,status,key=sheet,value=?{Sheet|D&D5E OGL,OGL|D&D5E Shaped,Shaped|PF2,PF2|} --show,status'))
        }
	...
	sendConcentrationMenu = function() {
		...
		listItems.push(makeTextButton('Use Concentration (5E)', concentration.useConcentration, '!cm --config,concentration,key=useConcentration,value='+!concentration.useConcentration + ' --show,concentration'))
		
		if (concentration.useConcentration) {
		    listItems.push(makeTextButton('Add Marker', concentration.autoAdd, '!cm --config,concentration,key=autoAdd,value='+!concentration.autoAdd+' --show,concentration'))	            
		    listItems.push(makeTextButton('Check for Save', concentration.autoRoll, '!cm --config,concentration,key=autoRoll,value='+!concentration.autoRoll+' --show,concentration'))  
		    listItems.push(makeTextButton('Notify', concentration.notify, '!cm --config,concentration,key=notify,value=?{Notify|Everyone,Everyone|Character,Character|GM,GM} --show,concentration'))
		 }
		
		if (concentration.autoRoll) {
		    listItems.push(makeTextButton('Wound Bar', concentration.woundBar, '!cm --config,concentration,key=woundBar,value=?{Wound Bar|Bar1,bar1|Bar2,bar2|Bar3,bar3} --show,concentration'))  
		    listItems.push(makeTextButton('Attribute', concentration.attribute, '!cm --config,concentration,key=attribute,value=?{Attribute|} --show,concentration'))  
    ...
    sendConditionsMenu = function(message) {
		...
		let	addButton = makeBigButton('Add Condition', '!cm --new,condition=?{Name}')
        let helpButton = makeImageButton('!cm --help,conditions',helpImage,'Help','transparent',18,'white')
    ...
    sendConditionMenu = function(key) {
        ...
        let listItems = []
        let markerDropdown = ''
        let helpButton = makeImageButton('!cm --help,condition',helpImage,'Help','transparent',18,'white')
		...
		let descriptionButton   = makeBigButton('Edit Description', '!cm --config,condition='+key+',key=description,value={{?{Description|'+condition.description+'}}} --show,condition='+key)
		let backButton          = makeBigButton('Back', '!cm --back')	 

		listItems.push(makeTextButton('Name', condition.name, '!cm --config,condition='+key+',key=name,value=?{Name}'))
		listItems.push(makeTextButton('Type', condition.type, '!cm --config,condition='+key+',key=type,value=?{Type|Condition,Condition|Spell,Spell} --show,condition='+key))
        ...
        if (condition.iconType == 'Token Condition') {
            listItems.push(makeTextButton('Icon', condition.icon, '!cm --config,condition='+key+',key=icon,value=?{Token Condition|} --show,condition='+key))				
        } else {     
	        listItems.push(makeTextButton('Icon', getDefaultIcon(condition.iconType,condition.icon), '!cm --config,condition='+key+',key=icon,value='+buildMarkerDropdown(condition.iconType)+' --show,condition='+key))				
        }
        ...
        listItems.push(makeTextButton('Targeted', condition.targeted, '!cm --config,condition='+key+',key=targeted,value='+!condition.targeted+' --show,condition='+key))
        if (condition.targeted) {
            listItems.push(makeTextButton('Targeted API', condition.targetedAPI, '!cm --config,condition='+key+',key=targetedAPI,value=?{Targeted API|Caster&Targets,casterTargets|Targets(Only),targets} --show,condition='+key))
        }
        listItems.push(makeTextButton('Concentration', condition.concentration, '!cm --config,condition='+key+',key=concentration,value='+!condition.concentration+' --show,condition='+key))
        listItems.push('<div style="margin-top:3px"><i><b>Adding Condition</b></i></div>' )
		listItems.push(makeBigButton('Add APIs', '!cm --show,condition='+key+',addAPI'))
        listItems.push('<div style="margin-top:3px"><i><b>Removing Condition</b></i></div>' )
		listItems.push(makeBigButton('Remove APIs', '!cm --show,condition='+key+',remAPI'))
    ...
    sendConditionAddAPIMenu = function (key) {
        ...
        listItems = [
		    makeTextButton('API', condition.addAPI, '!cm --config,condition='+key+',key=addAPI,value=?{API Command|} --show,condition='+key),
    		makeTextButton('Roll20AM', condition.addRoll20AM, '!cm --config,condition='+key+',key=addRoll20AM,value=?{Roll20AM Command|} --show,condition='+key),
		    makeTextButton('FX', condition.addFX, '!cm --config,condition='+key+',key=addFX,value=?{FX|} --show,condition='+key),
		    makeTextButton('Macro', condition.addMacro, '!cm --config,condition='+key+',key=addMacro,value=?{Macro|} --show,condition='+key),
		    makeTextButton('Persistent Macro', condition.addPersistentMacro, '!cm --config,condition='+key+',key=addPersistentMacro,value='+!condition.addPersistentMacro+' --show,condition='+key)
		]
    ...
    sendConditionRemAPIMenu = function (key) {
        ...
        listItems = [
		    makeTextButton('API', condition.remAPI, '!cm --config,condition='+key+',key=remAPI,value=?{API Command|} --show,condition='+key),
		    makeTextButton('Roll20AM', condition.remRoll20AM, '!cm --config,condition='+key+',key=remRoll20AM,value=?{Roll20AM Command|} --show,condition='+key),
		    makeTextButton('FX', condition.remFX, '!cm --config,condition='+key+',key=remFX,value=?{FX|} --show,condition='+key),
		    makeTextButton('Macro', condition.remMacro, '!cm --config,condition='+key+',key=remMacro,value=?{Macro|} --show,condition='+key)
		]
    ...
    targetedCondition = function (id, key) {
        ...
        let addButton    = makeImageButton('!cm --add,target,id='+id+',condition='+key,tagImage,'Targeted Icons','transparent',18,'white')
    ...
    targetedSpell = function (key) {
        ...
        let addButton    = makeImageButton(`!cm --add,condition=${key},duration=${condition.duration},direction=${condition.direction}`,tagImage,'Spell Targets','transparent',18,'white')
        title           += '<div style="display:inline-block;float:right;vertical-aligh:middle">'+addButton+'</div>'     
    ...
    targetedCaster = function (key,duration,direction,message) {
        ...
        let condition    = getConditionByKey(key)
        let addButton    = makeImageButton(`!cm --add,condition=${key},duration=${duration},direction=${direction},message=${message}`,tagImage,'Spell Caster','transparent',18,'white')
        title           += '<div style="display:inline-block;float:right;vertical-aligh:middle">'+addButton+'</div>'     
...
//*************************************************************************************************************		
	newCondition = function (name, type='Condition', concentration=false, description='None') {
		...
		if(!name){
			sendConditionsMenu('You didn\'t give a condition name, eg. <i>!cm --new,condition=Prone</i>.');
...
//*************************************************************************************************************	
    ...
    pauseTimer = function () {
        paused = !paused;
        makeAndSendMenu(paused ? 'Combat Timer has paused' : `Combat Timer has resumed`,' ', whisper);
    },
//*************************************************************************************************************
//ANNOUNCE 
//*************************************************************************************************************	  
...
//*************************************************************************************************************	
    ...
    makeBanner = function (command,title,previous) {
        let backButton = makeBigButton('Back', '!cm --back,'+previous)
        let helpButton = makeImageButton('!cm --help,'+command,helpImage,'Help','transparent',18,'white')
    ...
    handleSpellCast = function(msg) {
        ...
        if (status.autoAddSpells) {     
            ...
            if (typeof condition == 'undefined' && !getIgnoresByKey(key)) {
                ...
                let addSpellButton = makeBigButton(`Add Spell to Combat Master`, `!cm --spell,confirm=true,key=${key}`)
                let ignoreSpellButton = makeBigButton(`Ignore this Spell`, `!cm --spell,confirm=false,key=${key}`)
...
//*************************************************************************************************************	  
    ...
    checkInstall = function () {
        ...
        setDefaults();
        buildHelp();
        log(script_name + ' Ready! Command: !cm --main');
    },    
