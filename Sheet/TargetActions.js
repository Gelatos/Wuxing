...
// ======= Start Turn
// =================================================
function TargetStartTurn(targets) {
    ...
    roundResources = getAttrByName(target.charId, "roundResources");
    if (roundResources != "") {
        roundResources = roundResources.split(",");
        _.each(roundResources, function(id) {
            roundResource = GetCharacterAttribute(target.charId, GetSectionIdName(repeatingSection, id, "resourcecount"));
            if (roundResource != undefined) {
                roundResource.set("current", roundResource.get("max"));
            }
        });
    }
    ...
    if (raisedShieldObj != undefined && raisedShieldObj.get("current") == "1") {
        ...
        let raisedShieldAcObj = GetCharacterAttribute(target.charId, "gearEquippedShieldAc");
        if (raisedShieldAcObj != undefined) {

            raisedShieldObj.set("current", "0");
            let shieldAc = isNaN(parseInt(raisedShieldAcObj.get("current"))) ? 0 : parseInt(raisedShieldAcObj.get("current"));
            ...
            let acBarrier = GetCharacterAttribute(target.charId, "ac_barrier");
...
function GetTokenConditionInformation(targetData, showNameplate) {
    ...
    let output = {
        ...
        display: function() {
            if (this.conditions == "") {
                return `${this.formatHeader()}<div>No active conditions.</div>`;
            }
            
            return `${this.formatHeader()}${this.conditions}`;
        },
